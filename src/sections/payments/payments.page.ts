...
@Component({
  ...
  template: `
    <div class="container-app py-8">
      <h1 class="text-2xl font-semibold mb-4">{{ 'nav.payments' | translate }}</h1>
      <div class="card overflow-hidden">
        <table mat-table [dataSource]="rows()" class="w-full">
          ...
          <ng-container matColumnDef="method">
            <th mat-header-cell *matHeaderCellDef class="px-4 py-3 text-left text-sm font-medium">{{ 'payments.method' | translate }}</th>
            <td mat-cell *matCellDef="let r" class="px-4 py-3">{{ ('payments.method_' + r.method) | translate }}</td>
          </ng-container>
          ...
          <ng-container matColumnDef="date">
            <th mat-header-cell *matHeaderCellDef class="px-4 py-3 text-left text-sm font-medium">{{ 'common.date' | translate }}</th>
            <td mat-cell *matCellDef="let r" class="px-4 py-3">{{ r.date | date:'short' }}</td>
          </ng-container>
          <ng-container matColumnDef="status">
            <th mat-header-cell *matHeaderCellDef class="px-4 py-3 text-left text-sm font-medium">{{ 'common.status' | translate }}</th>
            <td mat-cell *matCellDef="let r" class="px-4 py-3">{{ ('payments.status_' + r.status) | translate }}</td>
          </ng-container>
